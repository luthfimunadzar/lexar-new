  ...
  <div>
    <div class="login-wrap">
      <div class="login-form">
        ...
        <b-alert show variant="danger" class="mb-4" v-if="anyError">{{
          errorMessage
        }}</b-alert>

        <form>
          <b-form-group :label="$t('emailAddress')" label-for="input-email">
            <b-form-input
              id="input-email"
          ...
          <b-button
            v-if="this.$route.query.incorp"
            variant="primary"
            type="submit"
            @click.prevent="incorpSubmit"
            class="w-100"
            :disabled="logging"
            >{{ $t("logIn") }}</b-button
          >
          ...
          <nuxt-link :to="localePath('register')" class="link-su">{{
            $t("needAccount")
          }}</nuxt-link>
          <nuxt-link :to="localePath('forgot-password')" class="link-fp">{{
            $t("forgotPassword")
          }}</nuxt-link>
        </form>
      </div>
